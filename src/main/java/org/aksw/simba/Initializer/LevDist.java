...
public class LevDist implements EnergyFunction {
	...
	/*
	 * This is the method for Levenshtein Distance. Levenshtein distance
	 * calculates similarity between two strings by comparing minimum number of
	 * words needed to convert one word to another
	 */
	private Double similarity(String ngram, String label) {
		String longer = ngram, shorter = label;
		if (ngram.length() < label.length()) { // longer should always have
												// greater length
			longer = label;
			shorter = ngram;
		}
