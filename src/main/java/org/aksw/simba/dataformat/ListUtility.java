...
public class ListUtility {
	public static void sortresults(List<ResultDataStruct> resultslist) {
		Collections.sort(resultslist, new Comparator<ResultDataStruct>() {
			...
			public int compare(ResultDataStruct o1, ResultDataStruct o2) {
				int date1Diff = o1.getExplainationScore().compareTo(
						o2.getExplainationScore());
				return date1Diff == 0 ? o1.getEnergyScore().compareTo(
						o2.getEnergyScore()) : date1Diff;
			}
		});
	}

	public static boolean Containselement(NGramStruct ngram,
	...
	public static ResultDataStruct ContainsResultDataStruct(String URI,
	...
	public static boolean isinBlackList(List<BlackListDataStruct> blacklist,
			String URI1, String URI2) {
		for (BlackListDataStruct blacknode : blacklist) {
			if (blacknode.getfirstURI().equals(URI1)
					&& blacknode.getsecondURI().equals(URI2)) {
				return true;
			}
			if (blacknode.getsecondURI().equals(URI1)
					&& blacknode.getfirstURI().equals(URI2)) {
				return true;
			}
		}
		return false;
