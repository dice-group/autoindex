...
public class NGramModel implements NGramInterface {
	...
	private void ngramhierarchy(int i, int height, int node, int first,
			int feed, int begin, int end, String keyword) {
		if (height == 4) {
			feed = 1; /*
		...
		if (!ListUtility.Containselement(middlenode, this.ngrammod)) {
		...
		ngramhierarchy((2 * i + 1), height + 1, node, first, feed, begin,
				end - 1, leftstring);
		ngramhierarchy((2 * i + 2), height + 1, node + 1, first, feed,
				begin + 1, end, rightstring);
	}
	...
	public List<NGramStruct> getNGramMod() {
		...
		for (int i = 0; i < this.ngrammod.size(); i++) {
			modngrams
					.set(this.ngrammod.get(i).getIndex(), this.ngrammod.get(i));
		}
		return modngrams;
	}
}