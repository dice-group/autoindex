...
import org.aksw.simba.dataformat.BlackListDataStruct;
...
public final class propagator implements PropagatorInterface {
	...
	/*
	 * Blacklist is used to keep track of the combination of nodes that already
	 * has been propagated
	 */
		...
		while (true) {
			...
			for (int i = 0; i < results.size(); i++) {
				...
				for (int j = 0; j < results.size(); j++) {
					...
					if (!ListUtility.isinBlackList(this.blacklist,
							results.get(i).getURI(), results.get(j).getURI())) {
						/*
						 * If the combination is not in blacklist, it activates
						 * the third node through fact node and puts it in the
						 * list
						 */
						flag = Activatefactnode(db, results.get(i),
								results.get(j), results);

						/* Insert the current pair in the blacklist */
						BlackListDataStruct combination = new BlackListDataStruct(
								results.get(i).getURI(), results.get(j)
										.getURI());
			...
			/*
			 * If all the combinations are in the blacklist and no new node is
			 * inserted, then break the loop
			 */
			if (mainflag == false) {
				break;
			}
	...
	public boolean ActivateTypeNode(GraphDatabaseService db,
			ResultDataStruct URI1, List<ResultDataStruct> results) {
		...
		try (Transaction ignored = db.beginTx();
				/*
				 * Executing the cypher query to find node attached to given two
				 * nodes
				 */
				Result result = db.execute("match (a:`" + URI1.getURI()
						+ "`)--c--(b:`"
						+ "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"
						+ "`) return c")) {
			Label activatedlabel1 = DynamicLabel.label(URI1.getURI());
			Label activatedlabel2 = DynamicLabel
			...
			if (run == true) {
				...
				for (Node node : IteratorUtil.asIterable(n_column)) {
					/*
					 * finding the third node connected to two nodes through
					 * fact node
					 */
					activatednewnode = thirdnode(node, activatednode1,
							activatednode2);
					...
					/*
					 * Checking if the new node is already activated before, if
					 * yes then the scores are incremented or else a new node is
					 * created and added to the list
					 */
					if (!((temp = ListUtility.ContainsResultDataStruct(
							activatednewnode.getProperty("URI").toString(),
							results)) == null)) {
						...
						temp.setEnergyScore(temp.getEnergyScore()
								+ URI1.getEnergyScore());
					} else {
						newnode = new ResultDataStruct(activatednewnode
								.getProperty("URI").toString(),
								union.size() + 5.00, URI1.getEnergyScore(),
								union);
	...
	public boolean Activatefactnode(GraphDatabaseService db,
			ResultDataStruct URI1, ResultDataStruct URI2,
			List<ResultDataStruct> results) {
		Node activatednode1 = null;
		Node activatednode2 = null;
		Node activatednewnode = null;
			...
			if (run == true) {
				...
				for (Node node : IteratorUtil.asIterable(n_column)) {
					if (URI1.isSubClassActivated()
							|| URI2.isSubClassActivated()) {
						if (URI1.isSubClassActivated()) {
							Set<Integer> union = ResultDataStruct.union(
									URI1.getColors(), URI2.getColors());
							URI1.setColors(union);
							URI1.setEnergyScore(URI1.getEnergyScore()
									+ URI2.getEnergyScore());
							...
							URI2.setEnergyScore(URI1.getEnergyScore()
									+ URI2.getEnergyScore());
					...
					/*
					 * This is to prevent recursion of activating the same
					 * triple again and again. This checks if the new activated
					 * node is part of previously activated tiple
					 */
					if (ListUtility.isinBlackList(this.blacklist,
							activatednewnode.getProperty("URI").toString(),
							activatednode1.getProperty("URI").toString())
							|| ListUtility.isinBlackList(this.blacklist,
									activatednewnode.getProperty("URI")
											.toString(), activatednode2
											.getProperty("URI").toString())) {
						continue;
					}
					...
					if (!((temp = ListUtility.ContainsResultDataStruct(
						...
						newnode = new ResultDataStruct(activatednewnode
								.getProperty("URI").toString(),
								(double) union.size(), URI1.getEnergyScore()
										+ URI2.getEnergyScore(), union);
	...
	private void ImageLink(GraphDatabaseService db, ResultDataStruct URI1) {
		...
		try (Transaction ignored = db.beginTx();
			...
			Label activatedlabel1 = DynamicLabel.label(URI1.getURI());
			Label activatedlabel2 = DynamicLabel
					.label("http://xmlns.com/foaf/spec/#term_depiction");
			...
			if (run == true) {
				...
				for (Node node : IteratorUtil.asIterable(n_column)) {
					...
					URI1.setImage(activatednewnode.getProperty("URI")
							.toString());
	...
	/*
	 * This method finds the third node that is connected with the two given
	 * nodes through fact node
	 */
	private static Node thirdnode(Node fact, Node node1, Node node2) {
		for (Relationship r : fact.getRelationships(Reltypes.Object_of)) {
			if (!(r.getOtherNode(fact).equals(node1) || r.getOtherNode(fact)
					.equals(node2)))
