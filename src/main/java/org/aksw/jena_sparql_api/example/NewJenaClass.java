...
//			qef = new QueryExecutionFactoryCacheEx(qef, cacheFrontend);
			...
			// Add pagination
			qef = new QueryExecutionFactoryPaginated(qef, 2);
			// Create a QueryExecution object from a query string ...
			QueryExecution qe = qef.createQueryExecution("Select ?s { ?s a <http://dbpedia.org/ontology/City> } Limit 6");
			
	
	
			// and run it.
			ResultSet rs = qe.execSelect();
//			System.out.println(ResultSetFormatter.toList(rs).size());
